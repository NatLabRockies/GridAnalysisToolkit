...
def get_generators_tech(scenario_dir, tech_map):
    ...
    gen_df.columns = pd.MultiIndex.from_tuples(
        [(tech_map[col], col) for col in gen_df.columns]
    )
...
def get_availability_tech(scenario_dir, tech_map):
    ...
    avail_df.columns = pd.MultiIndex.from_tuples(
        [(tech_map[col], col) for col in avail_df.columns]
    )
...
def calc_curtailment(gen_tech, avail_tech):
    ...
    curt_gen = (
        pd.concat([avail_tech[curt_tech], -1 * gen_tech[curt_tech]])
        .groupby(level=0)
        .sum()
    )
...
def get_entity_tech_aggregates(scenario_dir, tech_map, entity_map):
    ...
    gen_curt_tech.columns = pd.MultiIndex.from_tuples(
        [(entity_map[col[1]], col[0], col[1]) for col in gen_curt_tech.columns]
    )
...
def get_entity_tech_load_aggregates(
    scenario_dir, tech_map, gen_entity_map, region_entity_map
):
    ...
    regional_load_agg.columns = pd.MultiIndex.from_tuples(
        [(col, "Demand") for col in regional_load_agg.columns]
    )
    ...
    gen_tech_load_entity = pd.merge(
        regional_load_agg, gen_tech_load_agg, left_index=True, right_index=True
    ).sort_index(axis=1)
...
def get_line_load_data(scenario_dir, line_rating_map):
    ...
    loading = pd.DataFrame(
        {col: abs(flow[col] / line_rating_map[col]) for col in flow.columns.values},
        index=flow.index,
    )
