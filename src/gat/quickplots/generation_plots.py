...
def convert_units(data, from_unit="MWh", to_unit="GWh"):
    ...
    units = {"kWh": 1e3, "MWh": 1e6, "GWh": 1e9, "TWh": 1e12}
...
def plot_total_system_dispatch(
    scenario: BaseScenario,
    units="TWh",
    threshold=3,
    subpath="generation/system/total_dispatch",
    palette=None,
    backend=None,
):
    logger.info("plotting total system dispatch")
    dispatch = scenario.get_system_dispatch(
        include_load=False, include_charging=False
    ).sum()
...
@plot_function("BaseScenario", plot_type="generation")
def plot_monthly_system_dispatch(
    scenario: BaseScenario,
    subpath="generation/system/monthly_dispatch",
    palette=None,
    backend=None,
):
    logger.info("plotting monthly system dispatch")
    dispatch = scenario.get_system_dispatch(include_charging=False)
    ax, out_df = qp.plot_monthly_system_dispatch_stack(
        dispatch, return_frame=True, palette=palette, backend=backend
    )
...
@plot_function("BaseScenario", plot_type="generation")
def plot_total_area_dispatch(
    scenario: BaseScenario,
    subpath="generation/system/area_total_dispatch",
    palette=None,
    backend=None,
):
    logger.info("plotting total area dispatch")
    dispatch = scenario.get_area_dispatch(include_charging=False)
    ax, out_df = qp.plot_annual_area_dispatch_stack(
        dispatch, return_frame=True, palette=palette, backend=backend
    )
...
@plot_function("BaseScenario", plot_type="generation")
def plot_monthly_area_dispatch(
    scenario: BaseScenario, subpath="generation/area", palette=None, backend=None
):
    dispatch = scenario.get_area_dispatch(include_charging=False)

    for a in dispatch.columns.get_level_values(level="Area").unique():
        logger.info(f"plotting monthly dispatch for {a}")
        chart, out_df = qp.plot_monthly_system_dispatch_stack(
            dispatch[a], return_frame=True, palette=palette, backend=backend
        )
...
def plot_area_dispatch(
    ...
    for a in dispatch.columns.get_level_values(level="Area").unique():
        ...
        out_df = dispatch[a]
        chart = qp.plot_stacked_area_window(
            dispatch[a], palette=palette, backend=backend
        )
        from gat.quickplots.backends import get_backend
...
@plot_function("BaseScenario", plot_type="generation")
def plot_minmax_system_demand_windows(
    scenario: BaseScenario,
    window_delta=3,
    subpath="generation/system",
    palette=None,
    backend=None,
):
    ...
    for e in ["min", "max"]:
        ...
        if e == "min":
            chart, df_window = qp.plot_min_demand_window(
                dispatch,
            ...
            chart, df_window = qp.plot_peak_demand_window(
                dispatch,
                window_delta=window_delta,
                return_frame=True,
                palette=palette,
...
@plot_function("BaseScenario", plot_type="generation")
def plot_minmax_area_demand_windows(
    scenario: BaseScenario,
    window_delta=3,
    subpath="generation/area",
    palette=None,
    backend=None,
):
...
def plot_mean_hourly_area(
    ...
    for a in dispatch.columns.get_level_values(level="Area").unique():
        ...
        chart, df = qp.plot_mean_hourly_dispatch(
            dispatch[a], return_frame=True, palette=palette, backend=backend
        )
...
@plot_function("BaseScenario", plot_type="generation")
def plot_total_system_curtailment(
    scenario: BaseScenario,
    units="GWh",
    subpath="generation/system/total_curtailment",
    palette=None,
    backend=None,
):
    ...
    ax = plot_component_donut(
        tot_curt_scaled, unit=units, palette=palette, backend=backend
    )
...
@plot_function("BaseScenario", plot_type="generation")
def plot_monthly_system_curtailment(
    scenario: BaseScenario,
    subpath="generation/system/monthly_curtailment",
    palette=None,
    backend=None,
):
    ...
    chart, out_df = qp.plot_monthly_system_curtailment_stack(
        tot_curt, return_frame=True, palette=palette, backend=backend
    )
