...
class ScenarioPlotter:
    ...
    def annual_system_dispatch(self, **kwargs):

        system_dispatch = self.scenario.get_system_dispatch()
        ax = qp.plot_annual_system_dispatch_stack(system_dispatch, **kwargs)

        return ax

    def monthly_system_dispatch(self, **kwargs):

        system_dispatch = self.scenario.get_system_dispatch()
        ax = qp.plot_monthly_system_dispatch_stack(system_dispatch, **kwargs)
    ...
    def line_loading(self, start_date=None, end_date=None, **kwargs):
    ...
    def line_utilization(
        self, start_date=None, end_date=None, threshold=[75, 90, 95, 99], **kwargs
    ):
