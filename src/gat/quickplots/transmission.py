...
def plot_loading_ranked(loading_df, palette=None, backend=None, **kwargs):
    ...
    load_ave_sorted.plot.line(
        x="rank", y="Mean Load", ax=ax, color="Blue", label="Loading (ave)"
    )
    load_max_sorted.plot.line(
        x="rank", y="Max Load", ax=ax, color="Gold", label="Loading (max)", grid=True
    )
...
def plot_flow(flow, column, label, ax=None, annotate=True, palette=None, backend=None):
    ...
    flow[column].sort_values(ascending=False).reset_index().plot.line(
        y=column, ax=ax, label=label, color="Green", grid=True
    )

    if annotate:

        flow_stats = (
            flow[column].apply(lambda x: x if x >= 0 else 0.0).agg(["sum", "max"])
        )
        rflow_stats = (
            flow[column].apply(lambda x: abs(x) if x < 0 else 0.0).agg(["sum", "max"])
        )
...
def plot_ranked_series(series, ax=None, **kwargs):
    ...
    series.sort_values(ascending=False).reset_index()[[series_name]].plot.line(
        ax=ax, **kwargs
    )


def plot_hourly_boxplot(
    flow: pd.Series, ax=None, return_frame=False, palette=None, backend=None, **kwargs
):
    """
