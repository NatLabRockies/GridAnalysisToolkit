...
def plot_function(scenario_type: Optional[str] = "BaseScenario", plot_type: Optional[str] = 'system'):
    """
    ...
    def decorator(func: Callable) -> Callable:
        ...
        if not params:
            logger.warning(f"Plot function {func.__name__} has no parameters. It should accept a scenario object.")
            return func
...
def discover_internal_plots() -> None:
    ...
    for _, name, _ in pkgutil.iter_modules(quickplots.__path__, f"{quickplots.__name__}."):
...
def get_plots(scenario_type: Optional[str] = None, plot_type: Optional[str] = None) -> Dict[str, Dict[str, Any]]:
    """
    ...
    for name, info in _registered_plots.items():
        ...
        scenario_match = (scenario_type is None or
                          info["model_type"] == scenario_type or
                          info["model_type"] == "BaseScenario")
...
def get_plot_names(scenario_type: Optional[str] = None, plot_type: Optional[str] = None) -> List[str]:
    """
