...
class FileScenario(BaseScenario):
    ...
    def __init__(
        ...
        self._sienna_metadata = load_map(
            os.path.join(simulation_files, "metadata.json")
        )
        ...
        if not gen_area_map:
            if "Generator_region_mapping" in self._metadata:
                ...
                print(
                    "no generator region map detected, methods using get_area* may not be available."
                )
        ...
        if not load_area_map:
            if "Regions" in self._metadata:
                ...
                print(
                    "no load area map detected, methods using get_area* may not be available."
                )
    ...
    def get_line_rating_map(self):
        ...
        if "rate_to" in line_meta_df.columns:
            return (
                pd.DataFrame.from_dict(self._lines_meta)
                .transpose()[["rate", "rate_to"]]
                .rate.fillna("rate_to")
                .to_dict()
            )
        else:
    ...
    def list_aggregation_levels(self):
        if "aggregation_levels" in self._metadata.keys():
            ...
            print(
                "aggregation_levels not set, this might be an issue with your gat_metadata generation"
            )
    ...
    def set_aggregation_level(self, level: str):
        if level in self._metadata["aggregation_levels"]:
            ...
            self._gen_area_map = {
                k: v[level] for k, v in self._metadata["generator_area_maps"].items()
            }
            self._load_area_map = {
                k: v[level] for k, v in self._metadata["load_area_maps"].items()
            }
            # create new gen_id -> area map
            # create new load_id -> area map
        else:
            print(
                f"aggregation level {level} not found, try a value in list_aggregation_levels()"
            )
    ...
    def get_generation_capacity(self):
        ...
        df = pd.read_parquet(
            os.path.join(self._solution_data, "installed_capacity.pq.gz")
        ).T
