...
def test_loading_calc(handler_type):
    try:
        ...
        loading_data_simple = {col:100.0*abs(flow_sample[col]/line_rating[col]) for col in flow_sample.columns}
        ...
        assert np.allclose(loading_simple.values, loading_numpy.values, rtol=1e-5, atol=1e-8, equal_nan=True)
