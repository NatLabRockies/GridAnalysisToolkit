...
def map_system_capacity(scenario: SiennaScenario,
                        boundary_map=None,
                        marker_scale=0.5,
                        marker_alpha=0.8,
    ...
    gen_gdf = scenario.system.get_component_geo(scenario.config.system_config.generation_components)
    ...
    if boundary_map is None and len(available_areas)>0:
        ...
        area_gdf = scenario.config.area_lookups.get(lookup_group).get_layer_gdf(lookup_value)
    elif boundary_map is not None:
        lookup_group, lookup_value = scenario.list_aggregation_levels().get(boundary_map, default_area[1])
        area_gdf = scenario.config.area_lookups.get(lookup_group).get_layer_gdf(lookup_value)
    else:
        area_gdf = None
    ...
    gen_gdf['Technology'] = gen_gdf['name'].map(scenario._tech_map).map(scenario._tech_simple).fillna("Other")
    ...
    if area_gdf is not None:
        area_gdf.plot(facecolor=background_facecolor,edgecolor=background_edgecolor, alpha=background_alpha, ax=ax)
        crs = area_gdf.crs
    else:
        crs = gen_gdf.crs
    ...
    for i, cap in enumerate(marker_legend_cap):
        ...
        max_radius_in_data_units =  sqrt(max_cap*marker_scale) * data_range/display_range

        # Define marker and text positions
        marker_x = legend_x
        marker_y = legend_y + radius_in_data_units #(i * radius_in_data_units * 3)  # Space markers vertically
...
def map_transmission_capacity(
    ...
    cmap: Optional[Union[str, Dict[Any, str]]] = {345.0: 'red',230.0: 'green',138.0: 'blue', 69.0: 'purple',}, # can be a matplotlib cmap or user provided categorical.
    ...
    capacity_legend_values: List[float]=[10, 50, 100], # capacity values to show in the legend
    ...
    line_legend_spacing: Optional[float]=None, # vertical spacing between legend lines, auto-calculated if None
    ...
    arc_geo_meta = arc_geo.merge(bus_df, left_on='from_uuid', right_on='UUID',suffixes=['_arc','']).merge(bus_df, left_on='to_uuid', right_on='UUID', suffixes=['_from','_to'])
    arc_geo_meta = arc_geo_meta[['UUID_arc', 'base_voltage_from','base_voltage_to','area_from','area_to', 'name_from', 'name_to','geometry']]
    ...
    line_gdf = gpd.GeoDataFrame(line_geo[line_geo.available == available_only], crs='EPSG:4326')
    line_gdf = line_gdf[line_gdf['base_voltage_from'].between(min_voltage,max_voltage, inclusive='both')]
    line_gdf['linewidth'] = scale_marker(line_gdf['capacity'], max_size=max_linewdith, min_size=min_linewidth)
    ...
    legend_widths = [linewidth_scale*capacity/capacity_legend_values[-1]*max_linewdith for capacity in capacity_legend_values]
    ...
    if line_legend_spacing is None:
        line_legend_spacing = max_legend_width*spacing_factor*(bounds[3]-bounds[1])/1000

    # Calculate positions for all legend elements
    capacity_positions = [legend_y - (i + 1) * line_legend_spacing for i in range(len(capacity_legend_values))]
    voltage_title_y = legend_y - len(capacity_positions) * line_legend_spacing - line_legend_spacing * 1.5 - y_padding * voltage_section_spacing/2
    voltage_positions = [voltage_title_y - (i + 1) * line_legend_spacing - line_legend_spacing/2 for i in range(len(voltage_values))]
    ...
    legend_width = (line_length + 4*line_legend_padding + line_length) * (1 + legend_width_padding)  # Add extra width padding
    ...
    rect = FancyBboxPatch(
        (legend_x - line_legend_padding,
         legend_bottom),  # Start at the bottom of the legend
        legend_width,
        total_legend_height,
    ...
    voltage_line_width = max_legend_width * 0.6  # Slightly smaller than max capacity line
