...
def plot_generation_capacities(multi_scenario: "MultiScenario",**kwargs):
    ...
    for display_name, scenario in multi_scenario.scenarios.items():
        ...
        if area_filter is not None:
            ...
            if missing_areas:
                warn(f"MULTIPLOT: plot_generation_capacities - areas {missing_areas} not found in scenario '{display_name}'")

            if not valid_areas and area_filter:
                warn(f"MULTIPLOT: plot_generation_capacities - no valid areas from filter found for scenario '{display_name}'. Skipping filtering for this scenario.")
            elif valid_areas:
...
@plot_function("MultiScenario", plot_type="system")
def compare_system_capacities(multi_scenario:"MultiScenario", area_filter=None, **kwargs):
    """
...
@plot_function("MultiScenario", plot_type='system')
def compare_area_capacities(multi_scenario: "MultiScenario",area_filter=None, **kwargs):
    """
    ...
    for display_name in filtered_area_cap.index.get_level_values(level='Area').unique():
        ...
        if not area_filter or display_name in area_filter:
            ...
            fig, ax = plt.subplots()
            plot_stacked_component_bar(filtered_area_cap.loc[display_name]*1e-3, horizontal=True, ax=ax)
