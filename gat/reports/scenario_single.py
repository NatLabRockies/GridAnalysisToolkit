...
class SystemReportConfig(BaseModel):
    ...
    model_type: str
    report_type: Optional[str] = None  # 'generation', 'system', 'transmission', 'cost', 'emissions'
    ...
    def __init__(self, **data):
        ...
        if not data.get('output_plots'):
            logger.debug(f"No output_plots provided, discovering plots for model type: {self.model_type} and report type: {self.report_type}")
            self.output_plots = discover_available_plots(self.model_type, self.report_type)
    ...
    def save_config(self, path):
        ...
        try:
            ...
            logger.info(f"Configuration saved to {path} with {len(self.output_plots)} plot configurations")
        except Exception as e:
...
def discover_available_plots(model_type=None, plot_type=None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.debug(f"Discovering plots for model type: {model_type}, plot type: {plot_type}")

        # Get all plot functions that work with BaseScenario
        generic_plot_names = get_plot_names("BaseScenario", plot_type)
        logger.debug(f"Found {len(generic_plot_names)} generic plot names: {generic_plot_names}")
        ...
        logger.debug(f"Found {len(model_plot_names)} model-specific plot names: {model_plot_names}")
        model_plot_configs = []
...
def run(config: SystemReportConfig):
    ...
    try:
        ...
        if not config.output_plots:
            logger.warning("No plot configurations specified or discovered. Attempting to rediscover.")
            config.output_plots = discover_available_plots(config.model_type, config.report_type)
            if not config.output_plots:
                logger.error("No plot functions available. Check that plot plugins are properly registered.")
                return
        ...
        for plot_config in config.output_plots:
            try:
                ...
                if not plot_func:
                    logger.warning(f"Plot function {plot_name} not found in plugin registry")
                    continue
                ...
                logger.error(f"An unexpected error occurred while processing {plot_config.get('name', 'unknown plot')}: {e}")
                continue
...
def register_commands(init_group):
    ...
    available_plots = [v['plot_type'] for k, v in _registered_plots.items() if v['model_type'] != "MultiScenario"]


    @init_group.command("single")
    @click.option("--report-config", type=click.Path(), help="Path to a report configuration file")
    @click.option("--model-type", type=click.Choice(['sienna','plexos']), help="The underlying model type" )
    @click.option("--report-type", type=click.Choice(available_plots),
                  help="The type of report to generate")
    @click.option("--scenario-config", type=click.Path(exists=True), help="Path to scenario configuration file")
    @click.option("--system-path", type=click.Path(exists=True), help="Path to system data")
    @click.option("--simulation-paths", help="Comma-separated list of paths to simulation data")
    ...
    @click.option("--save-data/--no-save-data", default=True, help="Save plot data to CSV files")
    def single(report_config, model_type, report_type, scenario_config, system_path, simulation_paths, display_name, area_filter, output_fmt, output_path, save_config, save_data):
        ...
        if report_config:
            ...
            if not model_type:
                click.echo("Error: --model-type is required when creating a new configuration.", err=True)
                return
            ...
            if scenario_config:
                ...
                scen_config = ScenarioConfig(
                    ...
                    simulation_paths=simulation_paths.split(',') if simulation_paths else [],
                    display_name=display_name,
                    area_filter=area_filter.split(',') if area_filter else []
                )
            else:
                click.echo("Error: Either --scenario-config or --system-path must be provided.", err=True)
                return
...
def register_run_commands(run_group):
    ...
    @click.option("--output-path", type=click.Path(), help="Path for output files")
    @click.option("--save-data/--no-save-data", default=True, help="Save plot data to CSV files")
    def single(report_config, scenario_config, system_path, simulation_paths, display_name, area_filter, output_fmt, output_path, save_data):
        """Run the standard system report."""
        if report_config:
            logger.info(f"Loading report configuration from {report_config}")
            config = SystemReportConfig.from_config(report_config)
        elif scenario_config:
            logger.info("Loading scenario configuration and creating default report config")
            scen_config = load_config(scenario_config)
            config = SystemReportConfig(
                model_type=scen_config.model_type,
                scenario=scen_config
            )
        elif system_path:
            logger.info("Creating new scenario and report configuration from command line options")
            scen_config = ScenarioConfig(
                system_path=system_path,
                simulation_paths=simulation_paths.split(',') if simulation_paths else [],
                display_name=display_name,
            ...
            click.echo("Error: Must provide --report-config, --scenario-config, or --system-path.", err=True)
            return
